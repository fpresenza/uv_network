...
class r3dof4_kfi(kalman.KFi):
    ...
    def innovacion(self, z, y_j, I_j):
        ...
        y_j = sum(y_j)
        I_j = sum(I_j)
        dy = HtRinv.dot(z) + y_j - HtRinv.dot(hat_z) - I_j.dot(x)
        Y = HtRinv.dot(H) + I_j
        return dy, Y
...
if __name__ == '__main__':
    ...
    for t in tiempo[1:]:
        ...
        y_j[0], I_j[0] = kfi[0].informacion()
        y_j[1], I_j[1] = kfi[1].informacion()
        y_j[2], I_j[2] = kfi[2].informacion()

        dy[0], Y[0] = kfi[0].innovacion(z[0], y_j[1:], I_j[1:])
        dy[1], Y[1] = kfi[1].innovacion(
            z[1], [y_j[0], y_j[2]], [I_j[0], I_j[2]])
        dy[2], Y[2] = kfi[2].innovacion(z[2], y_j[:2], I_j[:2])
