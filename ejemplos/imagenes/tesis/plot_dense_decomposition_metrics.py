...
ax.set_xticks(np.arange(20, nmax + 1, 20))
ax.set_xticklabels(np.arange(20, nmax + 1, 20))
# ax.plot(nodes, np.mean(alpha, axis=1) / np.sqrt(2), label=r'$\alpha$', lw=1)
ax.plot(nodes, np.median(alpha, axis=1) / np.sqrt(2), label=r'$\alpha$', lw=1)
ax.fill_between(
    nodes,
    np.quantile(alpha, 0.25, axis=1) / np.sqrt(2),
    np.quantile(alpha, 0.75, axis=1) / np.sqrt(2),
    alpha=0.3)
# ax.legend(
