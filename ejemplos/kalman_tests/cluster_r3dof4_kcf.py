...
class r3dof4_kcf(kalman.KCF):
    ...
    def innovacion(self, z):
        ...
        Hx = H(self._x, self.i)
        HtRinv = np.matmul(Hx.T, self.Rinv)
        dy = np.matmul(HtRinv, dz)
        Y = np.matmul(HtRinv, Hx)
        return dy, Y
...
if __name__ == '__main__':
    ...
    for t in tiempo[1:]:
        ...
        kcf[0].actualizacion(t, [dy[0], dy[1]], [Y[0], Y[1]], [hat_x[1]], c=15)
        kcf[1].actualizacion(t, dy, Y, [hat_x[0], hat_x[2]], c=15)
        kcf[2].actualizacion(t, [dy[2], dy[1]], [Y[2], Y[1]], [hat_x[1]], c=15)
