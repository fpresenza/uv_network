    ...
    for k, t in steps[1:]:
        ...
        for i in nodes:
            ...
            if sum(Vi) > 1:
                ...
                pot_Ni = disk_graph.neighborhood_histeresis(
                    x, i, A[i].astype(bool), dmin, dmax)
                pot_Vi = Vi + pot_Ni
                # print(pot_Ni)
                pot_Ai = A[pot_Vi][:, pot_Vi]
                pot_xi = hatx[pot_Vi]
                pot_re = rigidity.eigenvalue(pot_Ai, pot_xi)
                if pot_re > 1e-3:
                    A[i] = pot_Ni.astype(int)
            else:
