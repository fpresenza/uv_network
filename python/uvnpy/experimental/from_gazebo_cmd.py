...
def run(arg, u, u_r, time):
    ...
    rover = integradores.EulerExplicito(
        lambda x, t, v: v, xi=(10, 0, 0.), ti=ti)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '-s', '--step',
        est='h', default=1e-3, type=float, help='paso de simulación')
    parser.add_argument(
        '-t', '--ti',
    ...
    parser.add_argument(
        '-u', '--file1', default='uav1_mavros_setpoint_velocity_cmd_vel.csv',
        type=str, help='nombre del archivo uav1')
    parser.add_argument(
        '-v', '--file2',
        default='uav1_mavros_setpoint_velocity_cmd_vel.csv',
        type=str, help='nombre del archivo uav2')
    parser.add_argument(
        '-r', '--file3',
        default='rover_mavros_setpoint_velocity_cmd_vel.csv',
    ...
    data_1 = np.loadtxt(
        arg.dir + arg.file1,
        delimiter=',', usecols=(0, 4, 5, 6, 9), skiprows=1)
    data_2 = np.loadtxt(
        arg.dir + arg.file2,
        delimiter=',', usecols=(0, 4, 5, 6, 9), skiprows=1)
    data_r = np.loadtxt(
    ...
    xp1 = TimePlot(
        time, lines, title='Multicopter 1 - state', color=color, label=label)
    ...
    label = [[('$x$', '$y$', '$z$')], [(r'$\phi$', r'$\Theta$', r'$\psi$')]]
    xp2 = TimePlot(
        time, lines, title='Multicopter 2 - state', color=color, label=label)
    ...
    if arg.animate:
        ani = Animation3D(
            time,
            xlim=(-15, 15), ylim=(-15, 15), zlim=(0, 10),
            save=arg.save, slice=50)
        ani.add_quadrotor(
