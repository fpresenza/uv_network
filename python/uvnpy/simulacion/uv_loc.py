...
from uvnpy.modelos.rover import Rover
...
def run(arg):
    ...
    Cam = collections.OrderedDict(
      [(r.id, []) for r in g.robots() if r.type == 'Drone']
    )
        ...
        if t > tc:
            ...
            for r in g.robots():
                if r.type == 'Rover':
                    ...
                    hat_Cov[r.id].append(metricas.sqrt_covar(r.cov_p()))
                elif r.type == 'Drone':
                    ...
                    hat_Cov[r.id].append(metricas.sqrt_covar(r.cov_mp()))
...
if __name__ == '__main__':
    ...
    parser.add_argument(
      '-g', '--rover',
      default=0, type=int, help='cantidad de rovers')
    parser.add_argument(
      '-a', '--drone',
      default=0, type=int, help='cantidad de drones')
    parser.add_argument(
      '-p', '--step',
    ...
    # g = graph.UnmannedVehicleGraph(
    #           directed=False, connect=graph.selective_proximity)
    # g.add_robots(arg.rover, deploy=lambda id: random_uniform_on_xy(10.))
    g.add_robots(
      arg.rover, deploy=lambda id: pos[id], object=Rover)
    g.add_robots(
      arg.drone, deploy=lambda id: pos[id],
      object=Rover, sensor_kw={'gps': True})
    ...
    #     axtitle=['UAV {}'.format(i+1) for i in range(arg.rover)],
    ...
    #     axtitle=['UGV {}'.format(i+1) for i in range(arg.rover)],
    #               xlabel=xlabel, ylabel=ylabel)]
    ...
    #     ani = spatial.Animation3D(t_ctr, xlim=(-35,35),
    #                    ylim=(-35,35), zlim=(0,20), save=arg.save, slice=3)
