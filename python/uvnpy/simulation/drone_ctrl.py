...
if __name__ == '__main__':
    ...
    parser.add_argument('-f', '--f_ctrl', default=20.0, type=float, help='frecuencia del controlador')
    ...
    uav = (
        Drone(1, motion_kw={'ti':arg.ti, 'pi':(1.,5.,5.), 'vi':(0,0.5,0), 'ai':(0.,0.,0.)}),
        Drone(2, motion_kw={'ti':arg.ti, 'pi':(0.,-5.,5.), 'vi':(0,0,0), 'ai':(0.,0.,0.)})
    )
    ...
    plot += [planar.GridPlot(shape=(1,1), title='Autocorrelación 1 - Innovación', xlabel='$\delta y$')]
    plot[1].axes[0,0].acorr(dyh, usevlines=False, color='r', linestyle="-", marker=".", linewidth=0.75, label='h')
    plot[1].axes[0,0].acorr(dyv, usevlines=False, color='g', linestyle="-", marker=".", linewidth=0.75, label='v')
    plot[1].fig.legend()
    ...
    plot += [planar.GridPlot(shape=(1,1), title='Autocorrelación 2 - Innovación', xlabel='$\delta y$')]
    plot[3].axes[0,0].acorr(dyh, usevlines=False, color='r', linestyle="-", marker=".", linewidth=0.75, label='h')
    plot[3].axes[0,0].acorr(dyv, usevlines=False, color='g', linestyle="-", marker=".", linewidth=0.75, label='v')
    plot[3].fig.legend()
