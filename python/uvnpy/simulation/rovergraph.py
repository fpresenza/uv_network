    ...
    for k, t in enumerate(tsim):
        ...
        if t > tc:
            for i in g.get_robots():
                ...
                hat_Cov[i].append(metrics.sqrt_covar(r.cov_p()))
            g.share_msgs()
            L.append(g.get_links())
            tc += Tc
            t_ctr.append(t)

    debug = dict([(r.id, {'f':r.filter.log, 'r':r.imu.accel.bias}) for r in g.robots()])

    return t_ctr, P, L, hat_P, hat_Cov, debug

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='')
    parser.add_argument('-n', '--n', default=5, type=int, help='cantidad de robots')
    ...
    # lines = (bx1, bx2, bx3, bx4, bx), (by1, by2, by3, by4, by), (bz1, bz2, bz3, bz4, bz)
    # color = ('r','g','b','y','k')
    # color = (color,)*4
    # ls = ('dotted', )*4 + ('--',)
    # ls = (ls,)*4
    ...
    # # color=[[('r','g')]], label=[[('horizontal','vertical')]], xlabel='$\delta y$')
    # label = 'x', 'y', 'z', 'range'
    # for lab, innov in zip(label, dy):
    #     plot[2].axes[0,0].acorr(innov, usevlines=False, linestyle="-", marker=".", linewidth=0.75, label=lab)
    # # ap1.axes[0,0].acorr(dyv, usevlines=False, color='g', linestyle="-", marker=".", linewidth=0.75)
    # plot[2].fig.legend()
