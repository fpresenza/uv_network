...
for k in range(N):
    ...
    th = x[k][:, 3]
    for i in range(n):
        axis = np.array([np.cos(th[i]), np.sin(th[i]), 0.0])
        cone = geometry.cone(p[i], axis, 5.0, np.pi/3)
        ax.add_collection3d(art3d.Poly3DCollection(cone, alpha=0.3))
